...
const EmployeeManagement = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const resSetting = await axiosClient.get('/api/auth/settings');
      setEmployees(res.data);
      setSettings(resSetting.data.shiftTimes);
    ...
    <div className="page-shell">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm mb-6 grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        >
          ...
          {settings.length > 0 && WORK_SHIFTS.map((s) => (
          ...
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <Field label="Ca làm việc">
                ...
                >
                  {settings && settings.length > 0 ? settings.map((s) => (
    <option key={s.id} value={s.id}>
      {s.label}
    </option>
  )) : WORK_SHIFTS.map((s) => (
